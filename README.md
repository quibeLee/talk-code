...
<img src="docs/images/logo-ai.svg" alt="Talk Code Studio Logo" width="140" />
...
![LangGraph4j](https://img.shields.io/badge/LangGraph4j-1.19.x-purple.svg)
...
![DeepSeek](https://img.shields.io/badge/DeepSeek-V4--Flash-4B7BEC.svg)
...
Talk Code Studio 是一个以工程化为目标的 AI 代码生成平台。  
...
- 做了`DeepSeek V4-Flash 适配`：用同一个 `DeepSeek V4-Flash` 模型承载普通对话与推理对话，通过 `thinking.type` 控制是否开启思考，并支持 `reasoning_effort` 调整推理强度。
...
context-compaction:
  # 触发自动压缩的 token 估算阈值
  # 建议设为所用模型最大上下文长度的 80%
  # 例如：DeepSeek V4-Flash 最大上下文 1M tokens → 800000
...
1. 查看你所使用的大模型的最大上下文长度（例如 DeepSeek V4-Flash 为 1M tokens）。
2. 将 `token-threshold` 设为最大上下文的 **80%**（预留 20% 给当前轮的输入输出）。
3. Token 估算方式：`总字符数 / 3`（中英混合场景的粗略估算）。

| 模型            | 最大上下文  | 推荐阈值（80%） |
| --------------- | ----------- | --------------- |
| DeepSeek V4-Flash | 1M tokens   | 800000          |
...
```bash
export Talk_CODE_COMPACTION_TOKEN_THRESHOLD=102400
export Talk_CODE_COMPACTION_MAX_SUMMARY_CHARS=300000
```
...
- 后端：`http://localhost:9011/api`

### 前端接口地址切换

当前暂时使用单体后端服务，前端开发代理和接口文档生成地址都指向单体服务端口 `9011`。

- 单体模式：后端启动根目录 Spring Boot 应用，接口地址为 `http://localhost:9011/api`。
- 微服务模式：前端应统一访问微服务网关 / 聚合入口，例如 `http://localhost:8080/api`，不要直接访问 `8124`、`8125`、`8127` 等单个服务端口。
- 开发代理切换：修改 `talk-code-frontend/vite.config.ts` 中 `/api` 的 `target`，单体为 `http://localhost:9011`，微服务为 `http://localhost:8080`。
- 接口代码生成切换：修改 `talk-code-frontend/openapi2ts.config.ts` 中的 `schemaPath`，单体为 `http://localhost:9011/api/v3/api-docs`，微服务为 `http://localhost:8080/api/v3/api-docs`。
- 临时覆盖方式：在 `talk-code-frontend/.env.local` 设置 `VITE_API_BASE_URL=http://localhost:9011/api` 或 `http://localhost:8080/api`；如需使用可视化编辑预览，优先保持默认同源 `/api` 并通过 Vite 代理切换后端。
